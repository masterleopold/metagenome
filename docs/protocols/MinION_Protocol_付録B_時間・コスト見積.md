...
## 🆕 推奨プロトコル比較表（2025年11月更新）

### Protocol 12 vs 従来アプローチ

**推奨**: 新規プロジェクトではProtocol 12（統合サンプル調製プロトコル）を使用してください。

| 項目 | Protocol 12（推奨・v2.1） | 従来アプローチ | 差分 |
|-----|----------------------|------------|------|
| **ワークフロー数** | 2（DNA + RNA） | 3-4 | -50% |
| **実験時間** | 15.5時間 | 16時間 | -3% |
| **Day 1** | 11.5時間 | 9時間 | +2.5時間（Step 2.5追加） |
| **Day 2** | 4時間 | 7時間 | -43% |
| **コスト/サンプル** | ¥162,000 | ¥152,000 | +¥10,000 (+6.6%) |
| **病原体カバー率** | 91/91 (100%真) | 87/91 (95.6%) | **4病原体の検出保証** |
| **環状DNA・ssDNA対応** | ✅ PCV2/PCV3/TTV/PPV | ❌ 検出不可 | **PMDA準拠達成** |
| **LOD（検出限界）** | 100-500 copies/mL | 50-500 copies/mL | スクリーニング十分 |
| **決定ポイント数** | 2 | 5 | -60% |
| **トレーニング時間** | 3日 | 5-7日 | -50% |
| **エラー率** | 2-3% | 5-8% | -60% |

**重要更新 (v2.1)**: Step 2.5（環状DNA直鎖化・ssDNA→dsDNA変換）追加により、従来アプローチでは検出不可能だったPCV2/PCV3/TTV/PPVの確実な検出が可能になりました。

### コスト内訳比較

#### Protocol 12（統合プロトコル v2.1）¥162,000
```
抽出・調製試薬:
├─ Zymo cfDNA/cfRNA Kit         ¥12,000
├─ NEBNext CpG Host Depletion   ¥8,000
├─ 🆕 環状DNA・ssDNA処理 ★NEW★
│  ├─ DNase I (RNase-free)      ¥1,500
│  ├─ Klenow Fragment (exo-)    ¥2,000
│  ├─ Random Hexamers           ¥1,000
│  └─ dNTP Set                  ¥500
├─ NEBNext Poly(A) Selection    ¥5,000  ← 単一RNA調製法
├─ NEBNext Ultra II cDNA        ¥15,000
├─ SUPERase•In, DNase等         ¥1,500
└─ 小計                         ¥46,500 (+¥5,000)

ライブラリ調製:
├─ LSK114 Kit (DNA用)           ¥25,000
├─ LSK114 Kit (RNA用)           ¥25,000
├─ AMPure XP Beads              ¥3,000
└─ 小計                         ¥53,000

シーケンシング:
├─ Flow Cell (1/24サンプル分)   ¥30,000
├─ Duplex Adapter               ¥2,000
└─ 小計                         ¥32,000

QC・その他:
├─ Qubit, TapeStation試薬       ¥2,500
├─ Pipette tips, tubes等        ¥1,000
└─ 小計                         ¥3,500

条件付きスピューマウイルス検査（5-10%サンプル）:
└─ PBMC分離 + Nested PCR        ¥1,125 (加重平均)

合計: ¥162,000 + ¥1,125 = ¥163,125 (実質)

**Step 2.5追加による効果**:
✅ PCV2/PCV3/TTV/PPV の検出が >95%検出率に向上（従来<5%）
✅ PMDA特別管理病原体（PCV2/PCV3）のコンプライアンス達成
✅ コスト増は最小限（+¥5,000、3.1%増のみ）
```

#### 従来アプローチ（可変コスト）¥152,000
```
DNAサンプル: ¥82,000
RNA poly(A)+: ¥95,000
RNA poly(A)-: ¥110,000 ← rRNA除去が高額
RNA amplicon: ¥130,000 ← 増幅法追加

加重平均: ¥152,000
（ただし、サンプルタイプにより±¥30,000の変動あり）
```

### 時間削減の内訳

| フェーズ | Protocol 12 | 従来 | 削減 |
|---------|-----------|-----|-----|
| RNA調製の決定 | 0分（自動） | 30分 | -30分 |
| rRNA除去 | 不要 | 2時間 | -2時間 |
| 増幅RT-PCR | 不要 | 6時間 | -6時間 |
| PBMC採取 | 条件付き | 全サンプル | -1.5時間平均 |
| **合計削減** | - | - | **3時間** |

### 主な利点

✅ **簡素化**: 2つのワークフローのみ（DNA + RNA）
✅ **予測可能**: コストと時間が固定
✅ **トレーニング**: 半分の時間で習得可能
✅ **エラー削減**: 決定ポイント減少で再現性向上
✅ **スループット**: 並行処理で効率化

### Protocol 11（高感度オプション）

LOD <50 copies/mLが必要な場合のみ使用:
...
**適応**: Protocol 12陽性後の確認、疫学的高リスク、研究目的
